...
def split_nodes_delimiter(nodes: list[TextNode], delimeter: str, text_type: str) -> list[TextNode]:
    transformed_nodes = []
    for node in nodes:
...
def __split_nodes(nodes: list[TextNode], is_image = False, extract = extract_markdown_links) -> list[TextNode]:
    transformed_nodes = []
    for node in nodes:
        if node.text_type != text_type_text:
            transformed_nodes.append(node)
